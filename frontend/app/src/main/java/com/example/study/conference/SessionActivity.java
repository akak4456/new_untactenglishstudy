package com.example.study.conference;
...
import com.example.study.conference.fragments.PermissionsDialogFragment;
import com.example.study.conference.openvidu.LocalParticipant;
import com.example.study.conference.openvidu.RemoteParticipant;
import com.example.study.conference.openvidu.Session;
import com.example.study.conference.utils.CustomHttpClient;
import com.example.study.conference.websocket.CustomWebSocket;
...
public class SessionActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        participant_name.setText(MyMemberInfo.getInstance().getMember().getName());
        session_name.setText("group"+gno);
        openvidu_url.setVisibility(View.INVISIBLE);
        openvidu_secret.setVisibility(View.INVISIBLE);
        session_name.setVisibility(View.INVISIBLE);
        participant_name.setVisibility(View.INVISIBLE);
    }
    ...
    public void buttonPressed(View view) {
        ...
        if (arePermissionGranted()) {
            ...
            System.out.println(android.util.Base64.encodeToString(("OPENVIDUAPP:" + OPENVIDU_SECRET).getBytes(), android.util.Base64.DEFAULT).trim());
    ...
    public void leaveSessionForDestroy(){
        try {
            if (start_finish_call.getText().equals(getResources().getString(R.string.hang_up))) {
                this.session.leaveSessionForDestroy(false);
            }else{
                this.session.leaveSessionForDestroy(true);
            }
            this.httpClient.dispose();
            this.session.remoteAllDispose();
            viewToDisconnectedState();
